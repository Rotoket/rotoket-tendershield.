...
const Analyzer: React.FC<AnalyzerProps> = ({ onAnalysisComplete, onOpenCalculator, onOpenGenerator }) => {
    ...
    const [legalReferencesMap, setLegalReferencesMap] = useState<Map<string, any[]>>(new Map());
                                ...
                                {result && (
                                    <div className="animate-fade-in space-y-6">
                                        ...
                                        <HeroVerdict
                                            ...
                                            mainProblem={result.executive_summary?.split('\n').find(line => 
                                            ...
                                            onGenerateRefusal={() => {
                                                // TODO: Генерация отказа
                                                logEvent('Analyzer', 'Запрошено формирование отказа', 'info');
                                            }}
                                        />
                                        ...
                                        <FinancialMetricsGrid
                                            ...
                                            financialAnalysis={result.financial_analysis}
                                        />

                                        {/* 4. RISK NARRATIVE - Повествовательные карточки рисков */}
                                        <RiskNarrative
                                            risks={(result.issues || []).map(issue => ({
                                              ...issue,
                                              legalReferences: legalReferencesMap.get(issue.title), // 🆕 Релевантные нормы
                                            }))}
                                            dealBreakers={result.deal_breakers}
                                            onGenerateProtocol={onOpenGenerator ? (dealBreakers) => {
                                              onOpenGenerator(dealBreakers, result.smart_questions);
                                            } : undefined}
                                            onViewKnowledge={(query) => {
                                              logEvent('Analyzer', `Переход в Базу знаний с запросом: ${query}`, 'info');
                                            }}
                                        />
                                        ...
                                        {/* 5. DEEP AUDIT CHECKLIST (ЕСЛИ ЕСТЬ) */}
